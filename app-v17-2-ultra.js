...
var KEY='rf_v17_2_premium_ultra';
var defaultState={tx:[],wallets:[],cards:[],goals:[],investments:[],budgets:[],subscriptions:[],plan:[],customCategories:[],profileName:'Guilherme',theme:'dark',sortTx:false,debts:[],assets:[],goalPro:[]};
...
on('backupBtn','click',function(){download('ramalho-finance-v17-2-ultra-backup.json',JSON.stringify(state,null,2),'application/json');});
...
document.addEventListener('click',function(e){
  ...
  var maps={tx:'tx',wallet:'wallets',card:'cards',goal:'goals',invest:'investments',budget:'budgets',sub:'subscriptions',plan:'plan',category:'customCategories',debt:'debts',asset:'assets',goalPro:'goalPro'};
...
function render(){
  ...
  renderBars(t);renderCategories();renderTopExpenses();renderMonthlyCompare();renderPremiumCenter(t,score,saving,cardPct);renderPremiumPlus(t,score);renderUltraHealth(t,score,saving,cardPct);renderNotifications(t);renderTimeline();renderGoalPro();renderDailyFlow();renderGoalInsights();renderAnalytics(t,score);renderCategoryRanking();renderDebts();renderAssets();renderAnnual(t);renderTransactions();renderWallets();renderCards();renderBudgets();renderSubs();renderPlan();renderCustomCategories();renderCalendar();renderGoals();renderInvests();renderChecklist(t);renderCloud();renderSummary();
}
...
function exportCsv(){var rows=['Mes,Data,Tipo,Categoria,Descricao,Valor,Recorrente'];state.tx.forEach(function(t){rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount,!!t.recurring].join(','));});download('ramalho-finance-v17-2-ultra.csv',rows.join('\n'),'text/csv');}

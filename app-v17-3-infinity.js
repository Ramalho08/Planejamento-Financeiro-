...
on('backupBtn','click',function(){download('ramalho-finance-v17-3-infinity-backup.json',JSON.stringify(state,null,2),'application/json');});
...
function render(){
  ...
  renderBars(t);renderCategories();renderTopExpenses();renderMonthlyCompare();renderPremiumCenter(t,score,saving,cardPct);renderPremiumPlus(t,score);renderUltraHealth(t,score,saving,cardPct);renderInfinityStats(t,score,saving,cardPct);renderStatistics(t,score);renderInvestPro();renderFavorites();renderRecentActions();renderNotifications(t);renderTimeline();renderGoalPro();renderDailyFlow();renderGoalInsights();renderAnalytics(t,score);renderCategoryRanking();renderDebts();renderAssets();renderAnnual(t);renderTransactions();renderWallets();renderCards();renderBudgets();renderSubs();renderPlan();renderCustomCategories();renderCalendar();renderGoals();renderInvests();renderChecklist(t);renderCloud();renderSummary();
}
...
function aiReport(){var t=totals(),s=t.income?Math.round(t.balance/t.income*100):0,cp=t.cardLimit?Math.round(t.cardUsed/t.cardLimit*100):0;return '<div class="alert"><strong>Relatório Finance AI Ultra</strong><p>Saldo mensal: '+brl(t.balance)+'<br>Taxa de economia: '+s+'%<br>Reserva ideal: '+brl(t.expense*6)+'<br>Uso do cartão: '+cp+'%<br>Assinaturas: '+brl(t.subTotal)+'<br>Categoria crítica: '+criticalCategory()+'<br><br>Recomendação: '+(t.balance>0?'direcione parte do saldo para metas e investimentos.':'reduza gastos variáveis antes de assumir novas parcelas.')+'</p></div>';}
function exportCsv(){var rows=['Mes,Data,Tipo,Categoria,Descricao,Valor,Recorrente'];state.tx.forEach(function(t){rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount,!!t.recurring].join(','));});download('ramalho-finance-v17-3-infinity.csv',rows.join('\n'),'text/csv');}

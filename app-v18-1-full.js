...
const defaultState={tx:[],wallets:[],cards:[],budgets:[],goals:[],investments:[],plan:[],assets:[],debts:[],subscriptions:[],favorites:[],recentActions:[],safeNotes:[],safeRoutine:[],customCategories:[],profileName:'Guilherme',theme:'dark',sortTx:false};
...
function totals(){
  ...
  return {income,expense,balance:income-expense,wallets,investments,assets,debts,subscriptions,cardUsed,cardLimit,gross,net};
}
...
on('backupBtn','click',()=>download('ramalho-finance-v18-1-full-backup.json',JSON.stringify(state,null,2),'application/json'));
...
document.addEventListener('click',e=>{
  ...
  const map={tx:'tx',wallet:'wallets',card:'cards',budget:'budgets',goal:'goals',investment:'investments',plan:'plan',asset:'assets',debt:'debts',sub:'subscriptions',favorite:'favorites',note:'safeNotes',routine:'safeRoutine'};
...
function render(){
  ...
  renderTransactions(); renderWallets(); renderCards(); renderBudgets(); renderSubscriptions(); renderGoals(); renderInvestments(); renderPlanning(); renderAssetsDebts(); renderAnalytics(); renderFavorites(); renderRecentActions(); renderSafeTools(); renderGlobalSearch(); renderMonthlySummary(); renderCloud();
}
...
function exportCsv(){const rows=['Mes,Data,Tipo,Categoria,Descricao,Valor'];state.tx.forEach(t=>rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount].join(',')));download('ramalho-finance-v18-1-full.csv',rows.join('\n'),'text/csv');}
...
function insertSample(){
  ...
  state.debts=[{id:id(),name:'Parcela antiga',total:600,paid:200}];state.subscriptions=[{id:id(),name:'Internet',value:99,category:'Assinaturas',due:10}];state.favorites=[{id:id(),name:'Mercado padrão',amount:80,type:'expense',category:'Alimentação'}];state.recentActions=[];state.safeNotes=[{id:id(),text:'Revisar orçamento no fim do mês',date:today()}];state.safeRoutine=[{id:id(),text:'Conferir cartão toda semana',done:false}];
}

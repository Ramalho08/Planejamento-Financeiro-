...
const KEY='rf_v22_professional_architecture';
...
on('backupBtn','click',()=>download('ramalho-finance-v22-professional-architecture-backup.json',JSON.stringify(state,null,2),'application/json'));
...
function exportCsv(){const rows=['Mes,Data,Tipo,Categoria,Descricao,Valor'];state.tx.forEach(t=>rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount].join(',')));download('ramalho-finance-v22-professional-architecture.csv',rows.join('\n'),'text/csv');}
...
function initStablePlusButtons(){
  ...
  if(clean&&!clean.dataset.ready){
    ...
    clean.addEventListener('click',function(){
      ...
      location.replace(location.href.split('?')[0]+'?v=22proarch&t='+Date.now());
    });
  }
  ...
  if(backup&&!backup.dataset.ready){
    ...
    backup.addEventListener('click',function(){
      download('ramalho-finance-v22-professional-architecture-backup.json',JSON.stringify(state,null,2),'application/json');
    });
  }
}
...
// V21.5.2 ROLLBACK STABLE GUARD
window.addEventListener('error',function(e){
  console.warn('Ramalho Finance guarded error:', e.message);
});
document.addEventListener('DOMContentLoaded',function(){
  if(typeof rebindNavigationButtons==='function')setTimeout(rebindNavigationButtons,200);
});

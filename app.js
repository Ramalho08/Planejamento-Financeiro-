...
var state=safe(localStorage.getItem('rf_v13_ultimate_safe'),{tx:[],wallets:[],cards:[],goals:[],investments:[],theme:'dark'});

function save(){localStorage.setItem('rf_v13_ultimate_safe',JSON.stringify(state));render();}
...
function totals(){
 ...
 return {income:income,expense:expense,balance:income-expense,wallets:wallets,investments:investments,net:wallets+investments+income-expense};
}
...
document.querySelectorAll('.tab,.sheet-link').forEach(function(btn){
 btn.addEventListener('click',function(){if(btn.dataset.page)setPage(btn.dataset.page);});
});
...
$('simulateGoalBtn').addEventListener('click',function(){
 var g=state.goals[0], monthly=Number($('monthlySave').value||0);
 $('goalSimulation').textContent=g&&monthly>0?'Guardando '+brl(monthly)+' por mês, você alcança "'+g.name+'" em '+Math.ceil((g.target-g.saved)/monthly)+' mês(es).':'Crie uma meta e informe um valor mensal.';
});

$('addInvestmentBtn').addEventListener('click',function(){
 state.investments.push({id:Date.now().toString(36),name:$('investmentName').value||'Investimento',amount:Number($('investmentAmount').value||0),rate:Number($('investmentRate').value||0)});
 $('investmentName').value='';$('investmentAmount').value='';$('investmentRate').value='';save();
});

$('simulateWealthBtn').addEventListener('click',function(){
 var total=state.investments.reduce(function(s,i){return s+i.amount;},0);
 var months=Number($('wealthYears').value||0)*12, aporte=Number($('wealthMonthly').value||0);
 for(var i=0;i<months;i++)total=(total+aporte)*1.008;
 $('wealthSimulation').textContent='Patrimônio estimado: '+brl(total);
});

$('generatePlanBtn').addEventListener('click',function(){
 var t=totals();
 $('assistantPlan').innerHTML='<div class="alert"><strong>Plano inteligente do mês</strong><p>1. Priorize despesas essenciais.<br>2. Tente guardar '+brl(Math.max(0,t.balance*.3))+'.<br>3. Sua reserva ideal é '+brl(t.expense*6)+'.<br>4. Revise categorias com maiores gastos.<br>5. Evite parcelamentos acima da sua renda mensal.</p></div>';
});

$('sampleBtn').addEventListener('click',function(){insertSample();save();});
$('exportCsvBtn').addEventListener('click',function(){var rows=['Mes,Data,Tipo,Categoria,Descricao,Valor,Recorrente'];state.tx.forEach(function(t){rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount,!!t.recurring].join(','));});download('ramalho-finance-v13.csv',rows.join('\\n'),'text/csv');});
$('backupBtn').addEventListener('click',function(){download('ramalho-finance-v13-backup.json',JSON.stringify(state,null,2),'application/json');});
...
$('clearBtn').addEventListener('click',function(){if(confirm('Apagar todos os dados?')){localStorage.removeItem('rf_v13_ultimate_safe');location.reload();}});
...
function makeAlerts(t,saving){var arr=[];if(t.income===0)arr.push('💡 Cadastre sua renda mensal.');if(t.balance<0)arr.push('⚠️ Seu mês está negativo.');if(saving>=25)arr.push('🚀 Boa economia neste mês.');if(t.expense>t.income*.75&&t.income>0)arr.push('🚨 Mais de 75% da renda comprometida.');if(!arr.length)arr.push('✅ Finanças equilibradas.');arr.push('🛡️ Reserva ideal: '+brl(t.expense*6));return arr;}
function renderBars(t){var data=[['Receitas',t.income],['Despesas',t.expense],['Carteiras',t.wallets],['Investimentos',t.investments]];var max=Math.max(1,t.income,t.expense,t.wallets,t.investments);$('bars').innerHTML=data.map(function(d){return '<small>'+d[0]+' — '+brl(d[1])+'</small><div class="bar"><span style="width:'+Math.round((d[1]/max)*100)+'%"></span></div>';}).join('');}
function renderCategories(){var cats={};currentTx().filter(function(t){return t.type==='expense';}).forEach(function(t){cats[t.category]=(cats[t.category]||0)+t.amount;});var max=Math.max(1,...Object.values(cats));$('categoryBars').innerHTML=Object.keys(cats).map(function(c){return '<small>'+c+' — '+brl(cats[c])+'</small><div class="bar"><span style="width:'+Math.round((cats[c]/max)*100)+'%"></span></div>';}).join('')||'<p>Sem despesas por categoria.</p>';}
function renderTransactions(){var q=($('searchTx').value||'').toLowerCase();var list=currentTx().filter(function(t){return t.description.toLowerCase().includes(q);});$('transactionList').innerHTML=list.map(function(t){return '<div class="row"><div><strong>'+t.description+'</strong><small>'+t.category+(t.installment?' • '+t.installment:'')+(t.recurring?' • recorrente':'')+'</small></div><strong class="'+(t.type==='income'?'positive':'negative')+'">'+(t.type==='income'?'+':'-')+' '+brl(t.amount)+'</strong></div>';}).join('')||'<p>Nenhum lançamento.</p>';}
...
function renderSummary(){var m={};state.tx.forEach(function(t){m[t.month]=m[t.month]||{income:0,expense:0};t.type==='income'?m[t.month].income+=t.amount:m[t.month].expense+=t.amount;});$('monthlySummary').innerHTML=Object.keys(m).map(function(k){return '<div class="row"><strong>'+k+'</strong><span>Receitas '+brl(m[k].income)+' • Despesas '+brl(m[k].expense)+'</span></div>';}).join('')||'<p>Sem resumo.</p>';}

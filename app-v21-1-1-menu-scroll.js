...
const KEY='rf_v21_1_1_menu_scroll';
...
on('backupBtn','click',()=>download('ramalho-finance-v21-1-1-menu-scroll-backup.json',JSON.stringify(state,null,2),'application/json'));
...
function exportCsv(){const rows=['Mes,Data,Tipo,Categoria,Descricao,Valor'];state.tx.forEach(t=>rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount].join(',')));download('ramalho-finance-v21-1-1-menu-scroll.csv',rows.join('\n'),'text/csv');}

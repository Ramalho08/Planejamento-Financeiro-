...
const KEY='rf_v18_4_1_fix_extract';
...
on('backupBtn','click',()=>download('ramalho-finance-v18-4-1-fix-extract-backup.json',JSON.stringify(state,null,2),'application/json'));
...
function parseExtractText(raw){
  ...
  lines.forEach(line=>{
    ...
    const parsed=parseExtractColumns(line,cols);
    if(!parsed)return;
    out.push({
      id:id(),
      approved:true,
      date:parsed.date||today(),
      month:(parsed.date||today()).slice(0,7),
      description:cleanDescription(parsed.description),
      amount:Math.abs(parsed.amount),
      type:parsed.type,
      category:guessCategory(parsed.description,parsed.type),
      source:'extrato',
...
function isIgnoredExtractLine(line){
  ...
  return !lower ||
    ...
    lower.includes('saldo disponível') ||
...
function parseExtractColumns(line,cols){
  let date='';
  let description='';
  const dateMatch=line.match(/(\d{1,2}[\/\-]\d{1,2}(?:[\/\-]\d{2,4})?|\d{4}[\/\-]\d{1,2}[\/\-]\d{1,2})/);
  if(dateMatch)date=normalizeDate(dateMatch[1]);

  const amountInfo=findBestMoney(line,cols);
  if(!amountInfo || amountInfo.value===0)return null;

  if(cols.length>=3){
    const descCols=cols.filter((c,i)=>i!==amountInfo.colIndex && !looksLikeDate(c) && !isValueMarker(c) && parseMoneyStrict(c)===null);
    description=descCols.join(' ').trim();
  }
  if(!description){
    description=line;
    if(dateMatch)description=description.replace(dateMatch[0],'');
    if(amountInfo.raw)description=description.replace(amountInfo.raw,'');
  ...
  const type=detectExtractType(line,description,amountInfo.value);
  const amount=Math.abs(amountInfo.value);
  let confidence=80;
  if(amount>1000000)confidence-=45;
  else if(amount>100000)confidence-=25;
  if(!dateMatch)confidence-=10;
  if(description.length<5)confidence-=10;
  if(amountInfo.colIndex===-1)confidence-=5;

  return {date,description,amount,type,confidence:Math.max(10,Math.min(99,confidence))};
}
function findBestMoney(line,cols){
  const candidates=[];
  cols.forEach((col,i)=>{
    const value=parseMoneyStrict(col);
    if(value!==null)candidates.push({raw:String(col).trim(),value,colIndex:i,score:scoreMoneyCandidate(String(col),i,cols.length)});
  });
  const regex=/(?:^|[\s;,\t])([DCdc+-]?\s*(?:R\$)?\s*\(?-?\d{1,3}(?:\.\d{3})*,\d{2}\)?|[DCdc+-]?\s*(?:R\$)?\s*\(?-?\d+\.\d{2}\)?|[DCdc+-]?\s*(?:R\$)?\s*\(?-?\d+,\d{2}\)?)(?=$|[\s;,\t])/g;
  let m;
  while((m=regex.exec(line))!==null){
    const raw=m[1].trim();
    const value=parseMoneyStrict(raw);
    if(value!==null)candidates.push({raw,value,colIndex:-1,score:scoreMoneyCandidate(raw,99,100)});
  }
  if(!candidates.length)return null;
  candidates.sort((a,b)=>b.score-a.score);
  return candidates[0];
}
function scoreMoneyCandidate(raw,index,total){
  let score=0;
  const s=String(raw||'');
  if(/\d+[,.]\d{2}/.test(s))score+=40;
  if(/R\$/i.test(s))score+=15;
  if(/^[\s+-]*[DCdc]/.test(s))score+=8;
  if(index===total-1)score+=10;
  if(index===total-2)score+=6;
  if(/\d{1,2}[\/\-]\d{1,2}/.test(s))score-=40;
  const abs=Math.abs(parseMoneyStrict(s)||0);
  if(abs>0&&abs<100000)score+=10;
  if(abs>1000000)score-=30;
  return score;
}
function parseMoneyStrict(text){
...
function renderExtractReader(){
  ...
  if(summary){const low=draft.filter(x=>(x.confidence||0)<60).length;summary.innerHTML=`<div class="professional-kpi"><div class="alert"><b>Detectados</b><p>${draft.length}</p></div><div class="alert"><b>Aprovados</b><p>${approved.length}</p></div><div class="alert"><b>Receitas</b><p>${brl(income)}</p></div><div class="alert"><b>Despesas</b><p>${brl(expense)}</p></div></div><div class="alert ${low?'extract-warn':'extract-ok'}"><span class="ai-chip">IA local V18.4.1</span> ${low?low+' item(ns) com baixa confiança. Revise valores altos antes de importar.':'Leitura concluída com boa confiança.'}</div>`;}
  if(list)list.innerHTML=draft.map(item=>`<div class="extract-row">
    ...
    <div><span class="extract-tag">Confiança: ${item.confidence||70}%</span> <small class="muted">Revise valores altos.</small></div><button class="delete" data-action="deleteExtract" data-id="${item.id}">Remover</button>
...
function exportCsv(){const rows=['Mes,Data,Tipo,Categoria,Descricao,Valor'];state.tx.forEach(t=>rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount].join(',')));download('ramalho-finance-v18-4-1-fix-extract.csv',rows.join('\n'),'text/csv');}

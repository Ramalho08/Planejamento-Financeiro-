...
const KEY='rf_v19_1_import_wizard';
...
on('approveAllSafeBtn','click',()=>{(state.extractDraft||[]).forEach(x=>{x.approved=!x.suspicious&&!x.duplicate;});save();});
...
on('backupBtn','click',()=>download('ramalho-finance-v19-1-import-wizard-backup.json',JSON.stringify(state,null,2),'application/json'));
...
function exportCsv(){const rows=['Mes,Data,Tipo,Categoria,Descricao,Valor'];state.tx.forEach(t=>rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount].join(',')));download('ramalho-finance-v19-1-import-wizard.csv',rows.join('\n'),'text/csv');}

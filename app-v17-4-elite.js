...
var KEY='rf_v17_4_infinity_elite';
var defaultState={tx:[],wallets:[],cards:[],goals:[],investments:[],budgets:[],subscriptions:[],plan:[],customCategories:[],profileName:'Guilherme',theme:'dark',sortTx:false,debts:[],assets:[],goalPro:[],favorites:[],recentActions:[]};
...
on('backupBtn','click',function(){download('ramalho-finance-v17-4-elite-backup.json',JSON.stringify(state,null,2),'application/json');});
...
function render(){
  ...
  renderBars(t);renderCategories();renderTopExpenses();renderMonthlyCompare();renderPremiumCenter(t,score,saving,cardPct);renderPremiumPlus(t,score);renderUltraHealth(t,score,saving,cardPct);renderInfinityStats(t,score,saving,cardPct);renderEliteForecast(t,score,saving,cardPct);renderWealthCenter(t);renderFutureBills();renderCardElite();renderCoachPlan(t,score);renderGlobalSearch();renderStatistics(t,score);renderInvestPro();renderFavorites();renderRecentActions();renderNotifications(t);renderTimeline();renderGoalPro();renderDailyFlow();renderGoalInsights();renderAnalytics(t,score);renderCategoryRanking();renderDebts();renderAssets();renderAnnual(t);renderTransactions();renderWallets();renderCards();renderBudgets();renderSubs();renderPlan();renderCustomCategories();renderCalendar();renderGoals();renderInvests();renderChecklist(t);renderCloud();renderSummary();
}
...
function renderEliteForecast(t,score,saving,cardPct){
  ...
  var daily=t.balance/30, m90=t.net+(daily*90), y365=t.net+(daily*365);
  ...
  el.innerHTML='<div class="mini-grid">'+
    ...
    '<div class="alert"><strong>30 dias</strong><p>'+brl(t.net+daily*30)+'</p></div>'+
    '<div class="alert"><strong>90 dias</strong><p>'+brl(m90)+'</p></div>'+
    '<div class="alert"><strong>365 dias</strong><p>'+brl(y365)+'</p></div>'+
  '</div>';
}
function renderWealthCenter(t){
  ...
  box.innerHTML='<div class="mini-grid"><div class="alert"><strong>Patrimônio bruto</strong><p>'+brl(gross)+'</p></div><div class="alert"><strong>Dívidas</strong><p>'+brl(t.debts||0)+'</p></div><div class="alert elite-card"><strong>Patrimônio líquido</strong><p>'+brl(net)+'</p></div><div class="alert"><strong>Reserva ideal</strong><p>'+brl(t.expense*6)+'</p></div></div>';
...
function renderCoachPlan(t,score){
  ...
  if((t.debts||0)>0)tips.push('Priorizar pagamento de dívidas com maior impacto.');
...
function renderFavorites(){
  ...
  el.innerHTML=state.favorites.map(function(f){
    return '<div class="alert"><strong>'+f.name+'</strong><p>'+f.category+' • '+brl(f.amount)+'</p><button class="favorite-btn" onclick="window.useFav(\\''+f.id+'\\')">Usar favorito</button><button class="delete-mini" data-action="favorite" data-id="'+f.id+'">Excluir</button></div>';
  }).join('')||'<p>Nenhum favorito.</p>';
}
window.useFav=function(id){
  var f=(state.favorites||[]).find(function(x){return x.id===id;});
  if(!f)return;
  quickTx(f.name,f.amount,f.type,f.category);
};
...
function aiReport(){var t=totals(),s=t.income?Math.round(t.balance/t.income*100):0,cp=t.cardLimit?Math.round(t.cardUsed/t.cardLimit*100):0;return '<div class="alert"><strong>Relatório Finance AI 4.0 Elite 4.0 Elite</strong><p>Saldo mensal: '+brl(t.balance)+'<br>Taxa de economia: '+s+'%<br>Reserva ideal: '+brl(t.expense*6)+'<br>Uso do cartão: '+cp+'%<br>Assinaturas: '+brl(t.subTotal)+'<br>Categoria crítica: '+criticalCategory()+'<br><br>Recomendação: '+(t.balance>0?'direcione parte do saldo para metas e investimentos.':'reduza gastos variáveis antes de assumir novas parcelas.')+'</p></div>';}
function exportCsv(){var rows=['Mes,Data,Tipo,Categoria,Descricao,Valor,Recorrente'];state.tx.forEach(function(t){rows.push([t.month,t.date,t.type,t.category,'"'+t.description+'"',t.amount,!!t.recurring].join(','));});download('ramalho-finance-v17-4-elite.csv',rows.join('\n'),'text/csv');}
